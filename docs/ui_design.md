...
### 3.2 インタラクション
- **スライド操作 (Slide to Act)**:
  - 「おやすみ開始 (SLEEP)」や「停止 (STOP)」などの重要アクションには、**スライダー UI**を採用。
  - 誤タップを防ぎ、ユーザーの明確な意思を確認するインタラクションとする。
...
### Wind Down Screen
- 「WIND DOWN」の文字。
- 現在時刻と、設定した起床時刻（WAKE UP TIME）をシンプルに表示。
- 下部に「SLIDE TO STOP」スライダーを配置。

### Blocking Screen (Shield)
- 全画面真っ黒。
- 中央に「MornDash」のロゴ、または制限メッセージ。
